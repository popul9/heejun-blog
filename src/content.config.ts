...
const roamer = defineCollection({
	loader: glob({ base: './src/content/roamer', pattern: '**/*.{md,mdx}' }),
	schema: ({ image }) =>
		z.object({
			title: z.string(),
...
const foodie = defineCollection({
	loader: glob({ base: './src/content/foodie', pattern: '**/*.{md,mdx}' }),
	schema: ({ image }) =>
		z.object({
			title: z.string(),
			description: z.string(),
...
export const collections = { roamer, foodie, developer, melophile };
