...
한마디로 Sonnet으로 아키텍처 디자인을 한다는 소리다. 단, 디버깅은 Opus로 한다 (이 야생마는 엣지 케이스를 탐험해낼 때는 미친 듯한 추리력을 가진다. 무섭다 😬).

한마디로 나는 적어도 시스템 디자인에 있어서는 약간 멍청한 모델을 의도적으로 고르게 되었다. 참고로 이 이론이 재밌게 느껴진다면 어떻게 "스마트"한 기능들을 직접 버림으로써 실패없는 제트기 시스템을 구현했는지 설명하는 다음 영상을 강추한다 :)
